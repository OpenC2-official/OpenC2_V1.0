...
def init(verilog_file, top_module_name, num_row, num_col, weight_bit_width, input_bit_width, std_cell_lib, dens, module_type):
    ...
    # adder_tree
        ...
        # init cells
        width_first_stage = std_cell_lib['half_adder'][0] + (weight_bit_width-1)*std_cell_lib['full_adder'][0] + std_cell_lib['adder_sign_extension'][0]
        # pyramid
        for key, value in cells.items():
            nums = re.findall(r'\d+', key)
            ...
            if re.match(r'^adder_\d+bit_\d+.ha_\d+$', key): 
                value[1][0] = area[0][0] + (stage-1)*width_first_stage+(stage-1)*(stage-2)/2
                value[1][1] = area[0][1] + area[1][1] - row_height - (2**stage+int(nums[1])*(2**(stage+1)))*row_height
            elif re.match(r'^adder_\d+bit_\d+.fa_\d+$', key): 
                value[1][0] = area[0][0] + std_cell_lib['half_adder'][0] + (int(nums[-1])-1)*std_cell_lib['full_adder'][0] + (stage-1)*width_first_stage+(stage-1)*(stage-2)/2
                value[1][1] = area[0][1] + area[1][1] - row_height - (2**stage+int(nums[1])*(2**(stage+1)))*row_height
