...
def generate_adder_module(bit_width):
    ...
    # circuit
    module_code += "  half_adder ha_0 (.a(a[0]), .b(b[0]), .s(s[0]), .cout(c[0]));\n"
    for i in range(1, bit_width):
        module_code += f"  full_adder fa_{i} (.a(a[{i}]), .b(b[{i}]), .cin(c[{i-1}]), .s(s[{i}]), .cout(c[{i}]));\n"
    # end
...
def generate_adder_sign_extension_module(bit_width):
    ...
    for i in range(1, bit_width):
        module_code += f"  full_adder fa_{i} (.a(a[{i}]), .b(b[{i}]), .cin(c[{i-1}]), .s(s[{i}]), .cout(c[{i}]));\n"
    module_code += f"  adder_sign_extension fa_{bit_width} (.sign(sign), .a(a[{bit_width-1}]), .b(b[{bit_width-1}]), .cin(c[{bit_width-1}]), .s(s[{bit_width}]));\n"
    # end
    module_code += "endmodule\n"
    module_code += "// End of module definition.\n\n"
    return module_code
...
def generate_adder_with_cin_module(bit_width):
    ...
    module_code += f"  wire [{bit_width-1}:0] c;\n"
    # circuit
    module_code += "  full_adder fa_0 (.a(a[0]), .b(b[0]), .cin(c_in), .s(s[0]), .cout(c[0]));\n"
    for i in range(1, bit_width):
        module_code += f"  full_adder fa_{i} (.a(a[{i}]), .b(b[{i}]), .cin(c[{i-1}]), .s(s[{i}]), .cout(c[{i}]));\n"
    # end
    module_code += "endmodule\n"
...
def generate_adder_tree_module(weight_bit_width, input_num):
    ...
    for i in range(input_num//2):
        ...
        module_code += f"(.sign(sign_weight), .a(in_{2*i}), .b(in_{2*i+1}), .s(sum_{weight_bit_width}bit_{i}));\n"
    # adder_mid_layers
    for k in range(log_input_num-2):
        for i in range(input_num//2**(k+2)):
            ...
            module_code += f"(.sign(sign_weight), .a(sum_{weight_bit_width+k}bit_{2*i}), .b(sum_{weight_bit_width+k}bit_{2*i+1}), .s(sum_{weight_bit_width+k+1}bit_{i}));\n"
    # adder_last_layer
    module_code += f"  adder_{psum_bit_width-1}bit_sign_extension adder_{psum_bit_width-1}bit "
    module_code += f"(.sign(sign_weight), .a(sum_{psum_bit_width-2}bit_{0}), .b(sum_{psum_bit_width-2}bit_{1}), .s(out));\n"
    # end
    module_code += "endmodule\n"
    module_code += "// End of module definition.\n\n"
    return module_code
...
def generate_dff_module(bit_width):
    ...
    for i in range(bit_width):
        module_code += f"  dff dff_{i} (.clk(clk), .rst_b(rst_b), .in(in[{i}]), .out(out[{i}]));\n"
    # end
    module_code += "endmodule\n"
    module_code += "// End of module definition.\n\n"
    return module_code
...
def generate_inverse_mux_module(bit_width):
    ...
    # circuit
    module_code += f"  inverter inv_sign (.in(sign), .out(sign_b));\n"
    for i in range(bit_width):
        module_code += f"  inverter inv_in_{i} (.in(in[{i}]), .out(in_b[{i}]));\n"
    for i in range(bit_width):
        module_code += f"  nand2 in_0_{i} (.in_0(in[{i}]), .in_1(sign_b), .out(in_0[{i}]));\n"
        module_code += f"  nand2 in_1_{i} (.in_0(in_b[{i}]), .in_1(sign), .out(in_1[{i}]));\n"
    for i in range(bit_width):
        module_code += f"  nand2 out_{i} (.in_0(in_0[{i}]), .in_1(in_1[{i}]), .out(out[{i}]));\n"
    # end
    module_code += "endmodule\n"
    module_code += "// End of module definition.\n\n"
    return module_code
...
def generate_adder_with_cin_for_acc_module(weight_bit_width, input_bit_width, input_num):
    ...
    # circuit
    module_code += f"  full_adder fa_0 (.a(a_0_to_{psum_bit_width-1}[0]), .b(b_0), .cin(c_in), .s(s[0]), .cout(c[0]));\n"
    for i in range(1, psum_bit_width):
        module_code += f"  full_adder fa_{i} (.a(a_0_to_{psum_bit_width-1}[{i}]), .b(b_1_to_{column_psum_bit_width-1}[{i-1}]), .cin(c[{i-1}]), .s(s[{i}]), .cout(c[{i}]));\n"
    for i in range(psum_bit_width, column_psum_bit_width):
        module_code += f"  full_adder fa_{i} (.a(a_{i}), .b(b_1_to_{column_psum_bit_width-1}[{i-1}]), .cin(c[{i-1}]), .s(s[{i}]), .cout(c[{i}]));\n"
    # end
    module_code += "endmodule\n"
    module_code += "// End of module definition.\n\n"
    return module_code
...
def generate_accumulator_module(weight_bit_width, input_bit_width, input_num):
    ...
    module_code += f"  nor2 nor_sign_out (.in_0(sign_weight), .in_1(sign_in), .out(sign_out_b));\n"
    module_code += f"  inverter inv_sign_out (.in(sign_out_b), .out(sign_out));\n"
    # nand2 & inverter for sign_in_msb
    module_code += f"  nand2 nand_acc (.in_0(sign_in), .in_1(in_msb), .out(sign_in_msb_b));\n"
    module_code += f"  inverter inv_acc (.in(sign_in_msb_b), .out(sign_in_msb));\n"
    # dff_psum
    module_code += f"  dff_{psum_bit_width}bit dff_psum (.clk(clk_psum), .rst_b(rst_b), .in(in), .out(psum));\n"
    # inverse_mux 
    module_code += f"  inverse_mux_{psum_bit_width}bit inverse_mux (.sign(sign_in_msb), .in(psum), .out(complement));\n"
    # mux_2_1
    module_code += f"  mux_2_1 mux (.s(sign_weight), .in_0(sign_in_msb), .in_1(complement[{psum_bit_width-1}]), .out(extension));\n"
    # tie low
    module_code += f"  tiel tie_low (.out(tie_to_low));\n"
    # adder_with_cin
    module_code += f"  adder_{column_psum_bit_width}bit_with_cin_for_acc adder (.c_in(sign_in_msb), .a_0_to_{psum_bit_width-1}(complement), "
    for i in range(input_bit_width):
        module_code += f".a_{psum_bit_width+i}(extension), "
    module_code += f".b_0(tie_to_low), .b_1_to_{column_psum_bit_width-1}(shift), .s(sum));\n"
    # dff_shift
    module_code += f"  dff_{column_psum_bit_width-1}bit dff_shift (.clk(clk_shift), .rst_b(rst_b), .in(sum[{column_psum_bit_width-2}:0]), .out(shift));\n"
    # dff_out
    module_code += f"  dff_{column_psum_bit_width}bit dff_out (.clk(clk_out), .rst_b(rst_b), .in(sum), .out(out));\n"
    # end
    module_code += "endmodule\n"
    module_code += "// End of module definition.\n\n"
    return module_code
...
def generate_sram_module(weight_bit_width, input_num):
    ...
    for j in range(input_num): 
        for i in range(weight_bit_width):
            module_code += f"  dcim_bitcell bitcell_{j}_{i} (.wl(wl[{j}]), .in_b(in_b[{j}]), .bl(bl[{i}]), .bl_b(bl_b[{i}]), .out(bitcell_out_{j}[{i}]));\n"
    # end
    module_code += "endmodule\n"
    module_code += "// End of module definition.\n\n"
    return module_code
...
def generate_sram_rw_module(weight_bit_width):
    ...
    # circuit
    for i in range(weight_bit_width): 
        module_code += f"  sram_rw rw_{i} (.bl(bl[{i}]), .bl_b(bl_b[{i}]), .pe(pe), .ysw(ysw), .ysr(ysr), .spe(spe), .se(se), .din(din[{i}]), .dout(dout[{i}]));\n"
    # end
    module_code += "endmodule\n"
    module_code += "// End of module definition.\n\n"
    return module_code
...
def generate_sram_with_rw_module(weight_bit_width, input_num):
    ...
    module_code += f".wl(wl), .in_b(in_b), .bl(bl), .bl_b(bl_b), "
    for j in range(input_num-1): 
    ...
    module_code += ".bl(bl), .bl_b(bl_b), .pe(pe), .ysw(ysw), .ysr(ysr), .spe(spe), .se(se), .din(din), .dout(dout)"
    module_code += ");\n"
    # end
    module_code += "endmodule\n"
    module_code += "// End of module definition.\n\n"
...
def generate_dcim_column_module(weight_bit_width, input_bit_width, input_num):
    ...
    module_code += f"  {sub_module_name} sram_with_rw ("
    ...
    module_code += f"  {sub_module_name} adder_tree ("
    ...
    module_code += f"  {sub_module_name} accumulator ("
...
def generate_nand_module(num): 
    ...
    module_code += f"  nand2 nand2_0 (.in_0(in_0), .in_1(in_1), .out(nand_0));\n"
    module_code += f"  inverter inv_0 (.in(nand_0), .out(and_0));\n"
    for i in range(1, num-2): 
        module_code += f"  nand2 nand2_{i} (.in_0(and_{i-1}), .in_1(in_{i+1}), .out(nand_{i}));\n"
        module_code += f"  inverter inv_{i} (.in(nand_{i}), .out(and_{i}));\n"
    module_code += f"  nand2 nand2_{num-2} (.in_0(and_{num-3}), .in_1(in_{num-1}), .out(out));\n"
    # end
    module_code += "endmodule\n"
    module_code += "// End of module definition.\n\n"
    return module_code
...
def generate_decoder_module(num_row):
    ...
    for i in range(log_num_row): 
        module_code += f"  inverter inv_for_input_{i} (.in(in[{i}]), .out(in_b[{i}]));\n"
    # nand
    for j in range(num_row): 
        module_code += f"  nand{log_num_row+1} nand_{j} ("
        temp = j
        for i in range(log_num_row): 
            if temp%2 == 0:
    ...
    for i in range(num_row): 
        module_code += f"  inverter inv_for_output_{i} (.in(out_b[{i}]), .out(out[{i}]));\n"    
    # end
    module_code += "endmodule\n"
    module_code += "// End of module definition.\n\n"
    return module_code
...
def generate_inverter_module(size): 
    ...
    # circuit
    for i in range(size): 
        module_code += f"  inverter inv_{i} (.in(in), .out(out));\n"
    # end
    module_code += "endmodule\n"
    module_code += "// End of module definition.\n\n"
    return module_code
...
def generate_driver_module(num_row, num_col):
    ...
    for j in range(stage-1): 
        for i in range(num_row):
            if size == 1:
                module_code += f"  inverter inv_stage{stage-1-j}_{i} (.in({in_name}[{i}]), .out({out_name}[{i}]));\n"
            else: 
                module_code += f"  inverter{size} inv{size}_stage{stage-1-j}_{i} (.in({in_name}[{i}]), .out({out_name}[{i}]));\n"
        size = math.ceil(size/4)
        in_name += '_b'
        out_name += '_b'
    for i in range(num_row):
        module_code += f"  inverter inv_stage0_{i} (.in(in[{i}]), .out({out_name}[{i}]));\n"
    # end
    module_code += "endmodule\n\n"
    return module_code
...
def generate_dcim_macro_module(num_row, num_col, weight_bit_width, input_bit_width): 
    ...
    module_code += f"  wire sign_out_b; \n"
    # control
    module_code += f"  control_for_{input_bit_width}bit_input control ("
    ...
    # decoder
    module_code += f"  decoder_{log_num_row}_to_{num_row} decoder (.enable(wl_enable), .in(wl_address), .out(wl_address_decoded));\n"
    # driver
    module_code += f"  driver_{num_row}x{num_col} driver_for_wl (.in(wl_address_decoded), .out(wl));\n"
    module_code += f"  driver_b_{num_row}x{num_col} driver_for_input (.in(in), .out(in_b));\n"
    # dcim column
    sub_module_name = f"dcim_column_{num_row}x{weight_bit_width}x{input_bit_width}"
    for j in range(num_dcim_column): 
        module_code += f"  {sub_module_name} dcim_column_{j} ("
